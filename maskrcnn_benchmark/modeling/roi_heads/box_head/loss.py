...
@registry.ROI_BOX_HEADS_LOSS.register("FastRCNNLossComputation")
...
@registry.ROI_BOX_HEADS_LOSS.register("FastRCNNLossWithAttrComputation")
class FastRCNNLossWithAttrComputation(FastRCNNLossComputation):
    ...
    def prepare_targets(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            labels.append(labels_per_image)
            attrs.append(attrs_per_image)
            regression_targets.append(regression_targets_per_image)

        return attrs, labels, regression_targets
    ...
    def subsample(self, proposals, targets):
        ...
        attrs, labels, regression_targets = self.prepare_targets(proposals, targets)
        sampled_pos_inds, sampled_neg_inds = self.fg_bg_sampler(labels)

        proposals = list(proposals)
        # add corresponding label and regression_targets information to the bounding boxes
        for attrs_per_image, labels_per_image, regression_targets_per_image, proposals_per_image in zip(
            attrs, labels, regression_targets, proposals
        ):
            proposals_per_image.add_field("labels", labels_per_image)
